...
const ProductsSection: React.FC<ProductsSectionProps> = ({ 
  ...
  const [touchStartX, setTouchStartX] = useState<{[key: string]: number | null}>({})
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        if (categoryTitle && categoryTitle !== 'Shop Now' && categoryTitle !== 'All Tattoos') {
        ...
        const productLimit = categoryTitle === 'All Tattoos' ? filteredProducts.length : 12
  ...
  useEffect(() => {
    ...
    const handleWishlistUpdate = () => {
      const updatedWishlist = JSON.parse(localStorage.getItem('bagichaWishlist') || '[]')
  ...
  const handleHeartClick = (productId: string, e: React.MouseEvent) => {
    ...
    const wishlist = JSON.parse(localStorage.getItem('bagichaWishlist') || '[]')
    const existingIndex = wishlist.findIndex((item: Product) => item.id === productId)
  ...
  return (
    <section className="products-section" aria-label="Products">
      <div className="products-content">
        {categoryImage && categoryTitle !== 'Shop Now' && categoryTitle !== 'All Tattoos' && !hideCategoryImage && (
        ...
        <div className={categoryTitle === 'All Tattoos' || useGridLayout ? 'products-items-grid' : 'products-items'}>
            ...
            return (
              <div key={product.id} className="wishlist-item" onClick={() => handleProductClick(product)}>
                ...
                <div className="wishlist-item-info">
                  <h3 className="wishlist-item-name">{product.title || product.name}</h3>
