...
const discountCodesCache = new SimpleCache<ShopifyDiscountCode[]>(
  5 * 60 * 1000,
  2000
);
...
async function fetchDiscountCodesForPriceRule(
  ...
  do {
    ...
    console.log(
      `📄 Fetching discount codes page ${page} for rule ${priceRuleId}...`
    );
    const response = await fetch(url, {
      method: "GET",
      headers,
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `❌ Failed to fetch discount codes page ${page}:`,
        errorText
      );
      break;
    }
    ...
    if (page > 100) {
      console.warn(
        `⚠️ Reached safety limit of 100 pages for discount codes (rule ${priceRuleId})`
      );
      break;
    }
  } while (pageInfo);
  ...
  console.log(
    `✅ Fetched ${allCodes.length} discount codes for price rule ${priceRuleId}`
  );
...
async function searchDiscountCodeInPriceRules(
  ...
  for (const priceRule of priceRules) {
    ...
    try {
      ...
      console.error(
        `❌ Error fetching discount codes for rule ${priceRule.id}:`,
        error
      );
...
function validatePriceRule(
  ...
  if (priceRule.prerequisite_subtotal_range?.greater_than_or_equal_to) {
    ...
    if (!isNaN(minAmount) && cartTotal < minAmount) {
      return {
        valid: false,
        error: `Minimum purchase of ₹${minAmount.toFixed(
          2
...
function calculateDiscount(
  ...
  if (valueType === "fixed_amount") {
    ...
    if (
      priceRule.target_selection === "specific" &&
      priceRule.entitled_variant_ids?.length
    ) {
    ...
    if (!lookupData) {
      ...
      try {
        ...
        if (lookupResponse.status === 404) {
          console.log(
            "❌ Lookup returned 404 - code not found via lookup endpoint"
          );
    ...
    return NextResponse.json(
      {
        ...
        error:
          error?.message ||
          "Failed to validate discount code. Please try again.",
      },
      { status: 500 }
    );
  }
