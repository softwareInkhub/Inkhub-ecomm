...
export default function ProductPage() {
      ...
      try {
        ...
        if (foundProduct) {
          ...
          const recentlyViewed = JSON.parse(
            localStorage.getItem("bagichaRecentlyViewed") || "[]"
          );
          const filteredViewed = recentlyViewed.filter(
            (item: Product) => item.id !== foundProduct.id
          );
          filteredViewed.unshift(foundProduct);
          // Keep only last 10 viewed items
          const updatedViewed = filteredViewed.slice(0, 10);
          localStorage.setItem(
            "bagichaRecentlyViewed",
  ...
  return (
    <div className="product-detail-page">
            ...
            <div className="product-specifications">
              <div className="spec-row">
                ...
                <span className="spec-value">
                  {(product as any).pattern || "Custom Design"}
                </span>
              </div>
              <div className="spec-row">
                <span className="spec-label">Type</span>
                <span className="spec-value">
                  {(product as any).fabric || "Temporary Tattoo"}
                </span>
              </div>
              <div className="spec-row">
                <span className="spec-label">Size</span>
                <span className="spec-value">
                  {(product as any).fit || "Medium Size"}
                </span>
              </div>
              <div className="spec-row">
                <span className="spec-label">Occasion</span>
                <span className="spec-value">
                  {(product as any).occasion || "Body Art"}
                </span>
              </div>
            </div>
          ) : (
            ...
            {similarProducts.map((item) => (
              ...
              >
                <div className="similar-product-image">
                  ...
                  />
                  <button
                    className={`similar-product-wishlist ${
                      isSimilarProductWishlisted(item.id) ? "active" : ""
                    }`}
                ...
                <div className="similar-product-info">
                  ...
                  <p className="similar-product-desc">
                    {(item as any).desc || item.description}
                  </p>
      ...
      <SwipeModal onAddToCart={handleAddToCart} onBuyNow={handleBuyNow} />
    </div>
  );
}
